...
class Bellmanford1:
    ...
    files = glob.glob('images/BellmanFord*')
    for f in files:
        os.remove(f)
    ...
    def constructGraph(self, vertices, edges, questionNo):
        ...
        #print('Debug Test--->curved_edges',curved_edges)
        straight_edges = list(set(G.edges()) - set(curved_edges))
        #print('Debug Test--->straight_edges', straight_edges)
        ...
        nx.draw_networkx_edges(G, pos, node_size=200,  edgelist=straight_edges)
        arc_rad = 0.25
        nx.draw_networkx_edges(G, pos, node_size=200, edgelist=curved_edges, connectionstyle=f'arc3, rad = {arc_rad}')
        #nx.draw_networkx_edge_labels(G, pos, font_size=10, edge_labels=nx.get_edge_attributes(G, 'weight'),label_pos=0.3)
        nx.draw_networkx_edge_labels(G, pos, font_size=10, edge_labels=edge_labels,label_pos=0.2,verticalalignment='center')
        path = 'images'
...
To answer it, you need to select the vertices that get updated in the particular iteration.<br/><br/>

<img src="images/bellman.png"><br><br>

For example, suppose that the question is as follows:  
Identify all the vertices that are updated at least once between 1 and 2 iteration (including 1 and 2 iteration changes) of the bellman ford algorithm,considering vertex 7 as the source  
Edges: - [(0, 6), (0, 3), (0, 2), (1, 7), (2, 0), (2, 5), (5, 1), (5, 3), (6, 0), (7, 2)]
Vertices:- 0 to 7
Options:
a) 0 (Correct) <br>
b) 1 (Correct) <br>
...
g) 6 (Correct) <br>
h) 7 <br>
Important Note: - Graph indication:	
Vertices (0,6) (6,0) (0,2) (2,0) are bi-directional represented with curved edges. The weights cannot be placed on the edges due to 
framework limitations. The weight nearest to the node is the weight of the edge to the node.<br>
0<--(2)--------------6 <br>
0-------------(-2)-->6 <br>
For Ex: -2 is the weight from 0 to 6 and 2 is the weight from 6 to 0.<p><\html>\""""
        else:
...
h) 7 <br>
Important Note: - Graph indication:	
Vertices for representation (0,6) (6,0) (0,2) (2,0) are bi-directional represented with curved edges. The weights cannot be placed on the edges due to 
framework limitations. The weight nearest to the node is the weight of the edge to the node.<br>
0<--(2)--------------6 <br>
0-------------(-2)-->6 <br>
For Ex: -2 is the weight from 0 to 6 and 2 is the weight from 6 to 0."""
...
+str(iterationStart)+""" and """+str(iterationEnd)+ """ iteration (including """+str(iterationStart)+ """ and """+str(iterationEnd)+ """
iteration changes) of the bellman ford algorithm,considering vertex """+str(sourceVertex)+  """ as the source """\
""" Hint: use this edgeList to solve :"""+str(edge_list)+"""<br>Important Note: - Graph indication:	
Vertices for representation (0,6) (6,0) (0,2) (2,0) are bi-directional represented with curved edges.<br> The weights cannot be placed on the edges due to 
framework limitations and will be in the white area within the curves.<br> The weight nearest to the node is the weight of the edge to the node.<br>
0<--(2)--------------6 <br>
0-------------(-2)-->6 <br>
For Ex: -2 is the weight from 0 to 6 and 2 is the weight from 6 to 0.</p>\""""
        return statement
    ...
    def print_answer(self, text, checked, feedback_index):
        if self.html == "HTML":
            answer_row = ",".join(["Option", str(int(checked)), str(text), "HTML", self.feedbacks[feedback_index], "HTML"])
        else:
